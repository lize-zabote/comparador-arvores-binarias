...
def gerar_grafico(caminho_csv, titulo_grafico, nome_arquivo_saida, escala_y_log=False, estruturas=None):
    try:
        ...
        for coluna in colunas:
            if df[coluna].sum() > 0:
                if coluna == 'B_Ordem_5':
                    plt.plot(df['Tamanho'], df[coluna], label=coluna, marker='o', markersize=3, linestyle='-', color='#1f77b4', alpha=0.7, linewidth=2)
                elif coluna == 'B_Ordem_10':
                    plt.plot(df['Tamanho'], df[coluna], label=coluna, marker='s', markersize=3, linestyle='--', color='#d62728', alpha=0.7, linewidth=2)
                else:
                    plt.plot(df['Tamanho'], df[coluna], label=coluna, marker='o', markersize=2, linestyle='-')
