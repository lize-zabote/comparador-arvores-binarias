...
void btree_adicionaChaveNo(NoB* no, NoB* novo, int chave) {
    ...
    // Contar só a inserção, não o loop
    for (int j = no->total - 1; j >= i; j--) {
        no->chaves[j + 1] = no->chaves[j];
        no->filhos[j + 2] = no->filhos[j + 1];
    }
    no->chaves[i] = chave;
    no->filhos[i + 1] = novo;
    contB++; // Contar só a operação de inserção
    if (novo != NULL) novo->pai = no;
    no->total++;
}
...
void btree_remover_interno(ArvoreB* arvore, NoB* no, int chave) {
    ...
    // Remover contagens excessivas, contar só a remoção
}
...
void fundir_com_proximo(ArvoreB *arvore, NoB *no, int idx)
{
    ...
    if (filho->filhos[0] != NULL)
    {
        for (int i = 0; i <= irmao->total; ++i)
        {
            ...
            contB++; // count null check
            if (filho->filhos[i + arvore->ordem])
                filho->filhos[i + arvore->ordem]->pai = filho;
        }
    }
...
int main()
{
    ...
    for (int N = 0; N <= MAX_N; N += PASSO)
    {
        ...
        for (int amostra = 0; amostra < NUM_AMOSTRAS; amostra++)
        {
            ...
            contAVL = 0;
            for (int i = 0; i < N; i++)
                avl_adicionar(avl, chaves[i]);
            soma_adicao_avl += contAVL;

            embaralhar(chaves, N);
            contAVL = 0;
            for (int i = 0; i < N; i++)
                avl_remover(avl, chaves[i]);
            ...
            contRubro = 0;
            for (int i = 0; i < N; i++)
                rb_adicionar(rb, chaves[i]);
            soma_adicao_rb += contRubro;

            embaralhar(chaves, N);
            contRubro = 0;
            for (int i = 0; i < N; i++)
                rb_remover(rb, chaves[i]);
            ...
            contB = 0; // Reinicia o contador para B_Ordem_1
            for (int i = 0; i < N; i++)
                btree_adicionar(b1, chaves[i]);
            soma_adicao_b1 += contB;

            embaralhar(chaves, N);
            contB = 0; // Reinicia o contador para B_Ordem_1
            for (int i = 0; i < N; i++)
                btree_remover(b1, chaves[i]);
            ...
            contB = 0; // Reinicia o contador para B_Ordem_5
            for (int i = 0; i < N; i++)
                btree_adicionar(b5, chaves[i]);
            soma_adicao_b5 += contB;

            embaralhar(chaves, N);
            contB = 0; // Reinicia o contador para B_Ordem_5
            for (int i = 0; i < N; i++)
                btree_remover(b5, chaves[i]);
            ...
            contB = 0; // Reinicia o contador para B_Ordem_10
            for (int i = 0; i < N; i++)
                btree_adicionar(b10, chaves[i]);
            soma_adicao_b10 += contB;

            embaralhar(chaves, N);
            contB = 0; // Reinicia o contador para B_Ordem_10
            for (int i = 0; i < N; i++)
                btree_remover(b10, chaves[i]);
